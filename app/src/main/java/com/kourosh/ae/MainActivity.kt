...
class MainActivity : Activity() {
    ...
    private val statusReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            intent.getStringExtra(KouroshAeVpnService.EXTRA_EXIT_IP)?.let { ip ->
                if (ip.isNotBlank()) {
                    coreExitIp = ip
                    coreExitCountry = ""
            ...
            if (intent.hasExtra(KouroshAeVpnService.EXTRA_TRAFFIC_TX)) {
                trafficTx = intent.getLongExtra(KouroshAeVpnService.EXTRA_TRAFFIC_TX, 0)
                trafficRx = intent.getLongExtra(KouroshAeVpnService.EXTRA_TRAFFIC_RX, 0)
                trafficSpeedTx = intent.getLongExtra(KouroshAeVpnService.EXTRA_TRAFFIC_SPEED_TX, 0)
                trafficSpeedRx = intent.getLongExtra(KouroshAeVpnService.EXTRA_TRAFFIC_SPEED_RX, 0)
                trafficMonthTx = intent.getLongExtra(KouroshAeVpnService.EXTRA_TRAFFIC_MONTH_TX, 0)
                trafficMonthRx = intent.getLongExtra(KouroshAeVpnService.EXTRA_TRAFFIC_MONTH_RX, 0)
                renderTrafficMonitor()
                renderHomeMetrics()
                return
            }
            when (intent.getStringExtra(KouroshAeVpnService.EXTRA_STATUS)) {
                KouroshAeVpnService.STATUS_CONNECTING -> showConnecting(
                    intent.getStringExtra(KouroshAeVpnService.EXTRA_DETAIL),
                    intent.getIntExtra(KouroshAeVpnService.EXTRA_PROGRESS, -1),
                )
                KouroshAeVpnService.STATUS_STARTING -> showStarting()
                KouroshAeVpnService.STATUS_SCANNING -> showScanning()
                ...
                KouroshAeVpnService.STATUS_CONNECTED -> beginVerification()
                KouroshAeVpnService.STATUS_FAILED -> {
                    ...
                    // is shown normally.
                    if (!advanceAutoScan()) {
                        showFailure(intent.getStringExtra(KouroshAeVpnService.EXTRA_DETAIL))
                    }
                }
                KouroshAeVpnService.STATUS_DISCONNECTED -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!AppAppearance.isNight(this)) setTheme(R.style.Theme_KouroshAe_Light)
        ...
        footerWave = OrbitFooterWave(this, palette, Strings.t("SECURED BY Kourosh-AE"))
    ...
    override fun onStart() {
        super.onStart()
        val filter = IntentFilter(KouroshAeVpnService.ACTION_STATUS)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
    ...
    private fun failFakeConnection() {
        ...
        startService(Intent(this, KouroshAeVpnService::class.java)
            .setAction(KouroshAeVpnService.ACTION_DISCONNECT))
    ...
    private fun refreshPublicIp(resetRetry: Boolean = true) {
        ...
        if (coreExitIp.isBlank() && isTunnelActive()) {
            KouroshAeVpnService.lastMeasuredExitIp()
    ...
    private fun updateNotificationHealth(ip: String? = null, ping: String? = null, country: String? = null) {
        ...
        startService(Intent(this, KouroshAeVpnService::class.java)
            .setAction(KouroshAeVpnService.ACTION_NOTIFICATION_HEALTH)
            .apply {
                ip?.let { putExtra(KouroshAeVpnService.EXTRA_NOTIFICATION_IP, it) }
                ping?.let { putExtra(KouroshAeVpnService.EXTRA_NOTIFICATION_PING, it) }
                country?.let { putExtra(KouroshAeVpnService.EXTRA_NOTIFICATION_COUNTRY, it) }
            })
    }
    ...
    private fun copyFullLog() {
        Thread({
            ...
            runOnUiThread {
                ...
                clipboard?.setPrimaryClip(ClipData.newPlainText(Strings.t("Kourosh-AE log"), text))
    ...
    private fun shareFullLog() {
        Thread({
            val result = runCatching {
                ...
                val target = File(dir, "kourosh_ae-log.txt")
                    ...
                    startActivity(
                        Intent.createChooser(
                            Intent(Intent.ACTION_SEND)
                                .setType("text/plain")
                                .putExtra(Intent.EXTRA_SUBJECT, "Kourosh-AE log (encrypted)")
                ...
                result.onFailure {
                    ...
                    startActivity(
                        Intent.createChooser(
                            Intent(Intent.ACTION_SEND)
                                .setType("text/plain")
                                .putExtra(Intent.EXTRA_SUBJECT, "Kourosh-AE Log Key")
                                .putExtra(Intent.EXTRA_STREAM, uri)
                                .addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION),
                            Strings.t("Log Key"),
                        )
    ...
    private fun openModeScreen() {
        ...
        content.addView(label(Strings.t("Choose how Kourosh-AE connects"), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(4); bottomMargin = dp(24) })
    ...
    private fun openSettingsScreen(animate: Boolean = true) {
        ...
        content.addView(expandableSection(Strings.t("PROTECTION"), id = "PROTECTION") { body ->
            ...
            body.addView(createToggleRow(
                ...
                preferences().edit().putBoolean(KouroshAeVpnService.AUTO_RECONNECT_PREF, it).apply()
            }, LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT,
            ).apply { topMargin = dp(8) })
        ...
        content.addView(expandableSection(Strings.t("ROUTING & DATA"), id = "ROUTING & DATA") { body ->
            ...
            body.addView(createToggleRow(
                ...
                preferences().edit().putBoolean(KouroshAeVpnService.LAN_BYPASS_PREF, it).apply()
            }, LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT,
            ).apply { topMargin = dp(8) })
            ...
            // see KouroshAeVpnService.applyIranBypass for why a rule inside any
            ...
            body.addView(createToggleRow(
                ...
                preferences().edit().putBoolean(KouroshAeVpnService.IRAN_BYPASS_PREF, it).apply()
        ...
        content.addView(expandableSection(Strings.t("ABOUT"), id = "ABOUT") { body ->
            ...
            body.addView(navRow(Strings.t("Source on GitHub"), iconRes = R.drawable.ic_github) {
                openLink("https://github.com/dkkdkddkfhi/Kourosh-AE")
            }, LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT,
            ).apply { topMargin = dp(8) })
            body.addView(navRow(Strings.t("Telegram Channel"), iconRes = R.drawable.ic_telegram) {
                openLink("https://t.me/timazadi")
            }, LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT,
            ).apply { topMargin = dp(8) })
            body.addView(label("Kourosh-AE ${appVersion()}", 11.5f, Sculpt.withAlpha(MUTED, 0.7f)).apply {
                gravity = Gravity.CENTER_HORIZONTAL
    ...
    private fun chooseTunnelType() {
        ...
        showChoiceSheet(
            ...
            subtitle = Strings.t("How Kourosh-AE carries your traffic"),
    ...
    private fun openTrafficMonitorScreen() {
        ...
        content.addView(label(Strings.t("Traffic carried by Kourosh-AE. Per-app attribution is not available from encrypted tunnel counters."), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
        ).apply { leftMargin = dp(4); bottomMargin = dp(24) })
    ...
    private fun openSplitTunnelScreen() {
        ...
        content.addView(header)
        content.addView(label(Strings.t("Choose which apps use Kourosh-AE. Changes apply next connection."), 14f, MUTED), LinearLayout.LayoutParams(
            ViewGroup.LayoutParams.MATCH_PARENT,
            ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    private fun connect(config: String) {
        ...
        // scan is running.
        armAutoScanWatchdog()
        startForegroundService(Intent(this, KouroshAeVpnService::class.java)
            .setAction(KouroshAeVpnService.ACTION_CONNECT)
            .putExtra(KouroshAeVpnService.EXTRA_CONFIG, config))
    }
    ...
    private fun startSessionTimer(restored: Boolean) {
        val serviceStart = KouroshAeVpnService.connectedSinceElapsed()
    ...
    private fun autoReconnectEnabled(): Boolean = preferences().getBoolean(
        KouroshAeVpnService.AUTO_RECONNECT_PREF,
        KouroshAeVpnService.AUTO_RECONNECT_DEFAULT,
    )
     ...
     * tunnel is up. [KouroshAeVpnService.lanBypassEnabled] is still the only
    ...
    private fun lanBypassEnabled(): Boolean = preferences().getBoolean(
        KouroshAeVpnService.LAN_BYPASS_PREF,
    ...
    private fun splitTunnelSummary(): String {
        ...
        return when (settings.mode()) {
            SplitTunnelSettings.Mode.ALL -> Strings.t("All apps use Kourosh-AE")
