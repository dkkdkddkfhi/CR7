...
class AppUpdater(private val activity: Activity) {
                    ...
                    .onSuccess { release ->
                        when {
                            ...
                            !isNewer(release.version, appVersion()) ->
                                showMessage(Strings.t("You're up to date"), Strings.tf("Kourosh-AE %s is installed", appVersion()))
    ...
    private fun announceUpdate(release: Release) {
        dialogBuilder()
            ...
            .setMessage(
                Strings.tf("Kourosh-AE %s has been released. You are on %s.", release.version, appVersion()) + "\n\n" +
    ...
    private fun dialogBuilder() = MaterialAlertDialogBuilder(
        activity,
        if (AppAppearance.isNight(activity)) R.style.ThemeOverlay_KouroshAe_AlertDialog
        else R.style.ThemeOverlay_KouroshAe_Light_AlertDialog,
    )
