...
object CoreConfig {
     ...
     * absent. See [KouroshAeVpnService.raiseOuterLeg].
     *
     * Written only after the byte threshold in
     * [KouroshAeVpnService.recordWorkingPlainTransport] — a handshake is not
     ...
     * [KouroshAeVpnService.armRegionPhase]. A plain Psiphon connect always takes
     ...
     * the dead `if (!psiphonVpnMode)` arm in `KouroshAeVpnService.onConnected` is what
     ...
     * front-end tun2socks dials; 1826 is the anytls sidecar's listener; the
    ...
    fun portRejection(port: Int): String? = when {
        ...
        port in RESERVED_PORTS -> "$port is used internally by Kourosh-AE"
        else -> null
    }
